...
def tableMakerTwo(a = 3, b = 63, c = 3, IncludeIllis = None):
    ...
    if not IncludeIllis == None:
        for i in IncludeIllis:
            tup = gimmeNumber.GimmeNumber(10 ** i, SHORT)
            tableRow(10 ** i, tup[0], tup[1], tup[2])
        titleA = "one " + gimmeNumber.GimmeNumber(10 ** IncludeIllis[0], SHORT)[0].title()
...
def listTableMaker(titleA, IncludeIllis = None):
    if titleA == "": 
        titleA = "one " + gimmeNumber.GimmeNumber(IncludeIllis[0], SHORT)[0].title()
    titleB = "one " + gimmeNumber.GimmeNumber(IncludeIllis[len(IncludeIllis) -1], SHORT)[0].title()

    for i in IncludeIllis:
        tup = gimmeNumber.GimmeNumber(i, SHORT)
...
def tableMakerOne(start = 1, length = 20 , step = 1, IncludeIllis = None):
    ...
    titleA = "one " + gimmeNumber.GimmeNumber(start, SHORT)[0].title()
    ...
    titleB = "one " + gimmeNumber.GimmeNumber(length, SHORT)[0].title()



    for i in range(start, length  + 1, step):
        gimme = (i)
        tup = (gimmeNumber.GimmeNumber(gimme, SHORT))
...
'''
    if (not short):
        s = s.replace("I(n) = 10^(3n + 3) [Short scale]", "I(n) = 10^(6n) [Long scale]")
    print(s)
    start= time.time()
    tableMakerOne(1, 20, 1)
    tableMakerOne(10, 100, 10, [1]) #the first two tables are formatted differently "e+06" instead of "e+6. I do not know why." 
